...
remaining authenticated with only a single Proxy Service. Users do not need to
have a direct connection to other clusters' Proxy Service.

## Prerequisites

(!docs/pages/includes/edition-prereqs-tabs.mdx!)

- Launch a second Teleport cluster in order to follow this guide. 

  We will call this cluster the **leaf cluster**. The cluster you typically use
  to follow guides will be called the **root cluster**. We will show you how to
  establish trust between the leaf cluster and the root cluster via Trusted
  Clusters.

- A host that is running a Teleport Node. See [Adding
  Nodes](../adding-nodes.mdx) for how to launch a Teleport Node in your cluster.
  
  This Node must be joined to the leaf cluster. If you launched your leaf
  cluster by following
  [Getting Started on a Linux Server](../../getting-started/linux-server.mdx),
  this step will already be complete.
