...
func caTargetToWatchKinds(targets []CertAuthorityTarget) ([]types.WatchKind, error) {
	...
	for _, target := range targets {
		kind := types.WatchKind{
			...
			// require a custom filter, which doesn't currently exist for CAs and has backwards
			// compatibility issues. Leaving this here so that if it is ever added in the future
			// it just works. For now, we will filter all subscriptions to only allow the types
			// defined in the CertAuthorityWatcherConfig.
...
func (c *caCollector) getResourcesAndUpdateCurrent(ctx context.Context) error {
	...
	for _, t := range c.Types {
		authorities, err := c.AuthorityGetter.GetCertAuthorities(t, false)
		if err != nil {
			return trace.Wrap(err)
		}
	...
	for _, ca := range cas {
		...
		c.fanout.Emit(types.Event{Type: types.OpPut, Resource: ca})
	}
	return nil
}
